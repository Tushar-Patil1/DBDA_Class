...
select month(date_published), count(id) from movie group by month(date_published) order by month(date_published);
...
where country = 'USA' or country = 'India' group by country
HAVING year = 2019;
...
select m.year, g.genre, count(g.movie_id) as number_of_movies
from genre as g
